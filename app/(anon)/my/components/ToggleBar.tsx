  ...
  return (
    <>
      <div className="w-full self-stretch inline-flex justify-between items-center">
        <div
          className={`flex-1 h-[50px] shadow-[0px_0px_0px_0px_rgba(148,148,148,0.25)] border-b border-secondary-e200 inline-flex flex-col justify-center items-center ${mySelected ? 'border-b-2 border-secondary-e200' : 'border-b-2 border-[var(--b400)]'}`}
          onClick={() => setMySelected(false)}
        >
          <Heart className="w-[26px] h-[26px]" />
        </div>
        <div
          className={`flex-1 h-[50px] relative border-b-2 ${mySelected ? 'border-[var(--b400)]' : 'border-b-2 border-secondary-e200'}`}
